...
> Draft of August 13, 2026. Preprint under active revision.
> Source: [github.com/cordiverse/paper](https://github.com/cordiverse/paper)
...
In particular, the initial effect context can be represented as (𝛾0 , idΓ ).
    We also write 𝜕 2 Γ = 𝜕Γ × (𝜕Γ → 𝜕Γ), and so on up the tower.
...
resulting types are the effect functions 𝔈Γ and their witnessed refinement 𝔈Γ∗ :
...
𝑔 with 𝑔 ∘ 𝑓 = idΓ meets the constraint at every state at once, and induces an element of 𝔈Γ∗ by
...
Proof.
   1. The unit lies in 𝔈Γ∗ since idΓ (𝛾) = 𝛾. For closure, take 𝑓, 𝑔 ∈ 𝔈Γ∗ and any 𝛾 ∈ Γ, and
...
𝑒′ . Then
     1. pr1 ∘ 𝑓 ′ = 𝑓 ∘ pr1 ;
     2. for each (𝛾, 𝜑) ∈ 𝜕Γ, the lifted inverse 𝑔′ ≔ pr2 (𝑒′ (𝛾, 𝜑)) and the inverse 𝑔 ≔ pr2 (𝑒(𝛾))
...
Theorem 15. Let 𝑒 ∈ 𝔈Γ∗ and write 𝑓 ≔ pr1 ∘ 𝑒. Fix (𝛾, 𝜑) ∈ 𝜕Γ, let (𝛿, 𝑔) = 𝑒(𝛾), and write (Δ, 𝑔′ )
...
The state is recovered exactly. The accumulator is restored as well, equivalently effectΓ (𝑒) ∈
  ∗
𝔈𝜕Γ , if and only if 𝑔 ∘ 𝑓 = idΓ ; and in every case (𝜑 ∘ 𝑔 ∘ 𝑓)(𝛾) = 𝜑(𝛾), so the soundness invariant
is preserved.
Proof. By Definition 12, Δ = (𝛿, 𝜑 ∘ 𝑔) and 𝑔′ = trackΓ (𝑔, 𝑓), so
                               𝑔′ (Δ) = (𝑔(𝛿), 𝜑 ∘ 𝑔 ∘ 𝑓) = (𝛾, 𝜑 ∘ 𝑔 ∘ 𝑓)
                                 ∗
using 𝑔(𝛿) = 𝛾. Membership in 𝔈𝜕Γ   requires this to equal (𝛾, 𝜑) at every input; taking 𝜑 = idΓ
...
equality of accumulators for every 𝜑. Finally (𝜑 ∘ 𝑔 ∘ 𝑓)(𝛾) = 𝜑(𝑔(𝛿)) = 𝜑(𝛾).                 □
    The lower triangle therefore closes only when the inverse witnessed at 𝛾 reverts 𝑓 at
                                                    ∗
every state, so effectΓ does not carry 𝔈Γ∗ into 𝔈𝜕Γ   . What holds in every case is agreement
                 ′
...
Theorem 16. Let 𝑒1 , ⋯, 𝑒𝑛 ∈ 𝔈Γ∗ be applied in order from (𝛾0 , idΓ ) and reverted in the reverse
order. Then
...
𝑔(𝛿) = 𝛾, so it preserves 𝜑(𝛾) by Theorem 7, whose hypothesis is exactly the witness of 𝔈Γ∗ .
...
Proof.
   1. The maps commuting with every generator of 𝔐(𝑒2 ) form a submonoid of Γ → Γ, since
       idΓ lies in it and 𝑓 ∘ 𝑓 ′ does where 𝑓 and 𝑓 ′ do. That submonoid contains the generators of
...
commutative.
    For effects induced by pairs (𝑓1 , 𝑔1 ) and (𝑓2 , 𝑔2 ), clause (1) is by Lemma 18(1) the commu­
tation of the four pairs 𝑓1 , 𝑓2 ; 𝑔1 , 𝑔2 ; 𝑓1 , 𝑔2 ; and 𝑔1 , 𝑓2 , and clause (2) holds outright, an induced
...
Theorem 20. Let 𝑒1 , ⋯, 𝑒𝑛 ∈ 𝔈Γ∗ be pairwise independent and applied in order from 𝛾0 . Write
𝑓𝑖 ≔ pr1 ∘ 𝑒𝑖 , let 𝛿𝑖 ≔ 𝑓𝑖 (𝛿𝑖−1 ) with 𝛿0 ≔ 𝛾0 , and let 𝑔𝑖 ≔ pr2 (𝑒𝑖 (𝛿𝑖−1 )) be the inverse 𝑒𝑖 yields
where it is applied. Fix 𝑗 and write 𝛿𝑖′ ≔ (𝑓𝑖 ∘ ⋯ ∘ 𝑓𝑗+1 )(𝛿𝑗−1 ) for the states of the sequence with
𝑒𝑗 omitted, so that 𝛿𝑗′ = 𝛿𝑗−1 . Then for every 𝑢 with 𝑗 ≤ 𝑢 ≤ 𝑛,
    1. 𝛿𝑢 = 𝑓𝑗 (𝛿𝑢′ ) and 𝑔𝑗 (𝛿𝑢 ) = 𝛿𝑢′ ;
                                           ′
...
Proof.
   1. The first equation is an induction on 𝑢. At 𝑢 = 𝑗 it reads 𝛿𝑗 = 𝑓𝑗 (𝛿𝑗−1 ), which is the
       definition of 𝛿𝑗 . For the inductive step, 𝛿𝑢+1 = 𝑓𝑢+1 (𝛿𝑢 ) = 𝑓𝑢+1 (𝑓𝑗 (𝛿𝑢′ )) = 𝑓𝑗 (𝑓𝑢+1 (𝛿𝑢′ )) =
            ′
       𝑓𝑗 (𝛿𝑢+1 ), the middle equality being clause (1) of Definition 19 for 𝑒𝑢+1 and 𝑒𝑗 , which are
       ...
       the one state it holds at:
                    𝑔𝑗 (𝛿𝑢 ) = (𝑔𝑗 ∘ 𝑓𝑢 ∘ ⋯ ∘ 𝑓𝑗+1 )(𝛿𝑗 ) = (𝑓𝑢 ∘ ⋯ ∘ 𝑓𝑗+1 )(𝑔𝑗 (𝑓𝑗 (𝛿𝑗−1 ))) = 𝛿𝑢′
       the last equality resting on 𝑔𝑗 (𝑓𝑗 (𝛿𝑗−1 )) = 𝛿𝑗−1 , which is the witness Definition 8 requires
       of 𝑒𝑗 at 𝛿𝑗−1 .
                                      ′
   2. By (1) the state 𝛿𝑖−1 is 𝑓𝑗 (𝛿𝑖−1 ), and 𝑓𝑗 ∈ 𝔐(𝑒𝑗 ), so clause (2) of Definition 19 for 𝑒𝑖 and
                             ′                   ′
      𝑒𝑗 gives pr2 (𝑒𝑖 (𝑓𝑗 (𝛿𝑖−1 ))) = pr2 (𝑒𝑖 (𝛿𝑖−1 )).                                            □
...
Corollary 21. Let 𝑒1 , ⋯, 𝑒𝑛 ∈ 𝔈Γ∗ be pairwise independent and applied in order from 𝛾0 , and let
...
plying 𝑔𝑗 at 𝛿𝑛 reaches 𝛿𝑛′ , the state the sequence with 𝑒𝑗 omitted reaches, and by Theorem 20(2)
...
components, which Section 4.4.2 carries to a trace of a whole system.
    Together, these constructions constitute revertible effects: each effect function in 𝔈Γ∗ explicitly
...
where get(𝑘) requires 𝑘 ∈ dom(𝜎) and set(𝑘, 𝑣) requires 𝑘 ∉ dom(𝜎) as preconditions.
                                   ∗
    Notably, set(𝑘, 𝑣) has type 𝔈Σ   , precisely an effect function on the coeffect context. We can
...
Definition 24. A coeffect at a key 𝑘 is a triple (𝒱︀𝑘 , ≃, 𝒜︀𝑘 ), where 𝒱︀𝑘 is the value type of Defin­
                                                       𝑘
ition 22, ≃ is an equivalence relation on 𝒱︀𝑘 up to which values at 𝑘 are compared (Section 3.3.2),
          𝑘
...
transforms 𝜎 to 𝜎′ can be classified by a specification 𝑑 ∈ 𝔇Σ according to whether 𝑑’s satis­
faction status is altered:
Definition 26. Given a coeffect specification 𝑑 ⊆ 𝐾 and states 𝜎, 𝜎′ ∈ Σ, define:
                                            activating if 𝜎 ⊭ 𝑑 ∧ 𝜎′ ⊧ 𝑑
                         notify𝑑 (𝜎, 𝜎′ ) ≔ deactivating if 𝜎 ⊧ 𝑑 ∧ 𝜎′ ⊭ 𝑑                      (26)
                                           {neutral      otherwise
...
It can be represented as a pair (𝜌, 𝜎), where:
    ...
    • 𝜎 : (𝑟 : 𝑅) ⇀ 𝒱︀𝑟 is the dependency table, a partial dependent function from realm identi­
...
grained control, enabling customized isolation for specific components; set remains an effect
            ∗
function (𝔈Σ  iso ) and thus inherits revertibility, whereas isolate needs none, deriving a context
...
where the three projections are:
   ...
   • Σ: the coeffect context carrying dependency information.
    Under this definition, effect maps 𝔈Γ∞ to itself, unifying the 𝜕-tower into a single self-
...
Definition 37. Read Definition 8 up to ≃: an 𝑒 ∈ 𝔈Γ lies in 𝔈Γ∗ when 𝑒 respects ≃ as a map Γ →
𝜕Γ and, writing (𝛿, 𝑔) = 𝑒(𝛾), for every 𝛾 ∈ Γ
   1. 𝑔(𝛿) ≃ 𝛾;
   2. 𝑔 respects ≃.
Taking ≃ to be equality on Γ recovers Definition 8.
Lemma 38. With 𝔈Γ∗ read as in Definition 37, every equality of states asserted in Section 3.1 holds
...
speaks of.
                                                                          𝒜︀
Definition 41. The coeffect-mediated effect functions form the least set 𝔈Σ  ⊆ 𝔈Σ that contains the
unit 𝜂Σ and is closed under the following: for a key 𝑘, an operation 𝑎 ∈ 𝒜︀𝑘 , an argument 𝑥 :
𝑋𝑎 , and a family (𝑒𝑏 )𝑏∈𝐵 of members,
                         𝑎
...
occurring in a member are the ones its stages perform, over every choice of outcome.
                            𝒜︀
Theorem 42. Let 𝑒1 , 𝑒2 ∈ 𝔈Σ   and let every key at which operations of both occur be commu­
tative (Definition 39). Then 𝑒1 and 𝑒2 are independent (Definition 19).
...
representing a triple (𝑑, 𝑝, 𝑒), where:
   ...
   • 𝑒 : 𝔈Γ∗ is the witnessed effect function of Definition 8, defining the effects contributed
...
tuple ⟨𝑑, 𝑝, 𝑒, 𝜋, 𝜎, 𝜏 , 𝜃⟩, where
   • 𝑑 : 𝔇Γ , 𝑝 : 𝔓Γ , and 𝑒 : 𝔈Γ∗ are the coeffect specification, provision, and effect function of
      Definition 43;
...
𝛿 = 𝑓(𝛾),
   ...
   2. (Reads.) two states agreeing on 𝜎𝑛 , on the restrictions 𝜎𝑚 |𝑑𝑛 for every 𝑚 ∈ dom(𝐹𝛾 ), and
...
where 𝑖 : 𝔈Γiter∗ is the remaining effect iterator (Definition 51 below), 𝑔 : Γ → Γ the accumulator
...
the wider state space as
         ...
         quiet(𝛾) ≔ ∀𝑛 ∈ dom(𝐹𝛾 ). target𝑛 (𝛾) = 𝜔𝑛         if 𝜃𝑛 = 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔𝑛 )            (45)
                                  { ⊥                       otherwise
...
Definition 51. Define the effect iterator 𝔈Γiter and witnessed effect iterator 𝔈Γiter∗ as the following
recursive types:
                      𝔈Γiter ≔ 𝜇ℑ. Γ → Γ × (Γ → Γ) × 𝖬𝖺𝗒𝖻𝖾(ℑ)
                     𝔈Γiter∗ ≔ 𝜇ℑ. (𝑒 : Γ → Γ × (Γ → Γ) × 𝖬𝖺𝗒𝖻𝖾(ℑ))                                (47)
...
The witness is read at the ≃ of Definition 33, as Definition 37 reads that of 𝔈Γ∗ : an 𝑖 ∈ 𝔈Γiter lies in
𝔈Γiter∗ when 𝑖 respects ≃ and each 𝑔 it yields respects ≃ and satisfies the clause above. A triple
...
𝜑 ∘ 𝑔1 ∘ ⋯ ∘ 𝑔𝑘 naturally recovers effects in LIFO order when applied. Because effectiter
                                                                                      Γ lands in
the same 𝜕Γ → 𝜕 2 Γ as effectΓ does, an iterator is an effect in its own right and can be used
...
yield operator [43], so the model maps directly onto the generators they already provide.
    In the calculus, the 𝑒𝑛 of Definition 44 is read at 𝔈Γiter∗ from here on, and replacing the atomic
...
raise in place of yielding a triple:
                  𝔈Γfail ≔ 𝜇ℑ. Γ → 𝖤𝗂𝗍𝗁𝖾𝗋(Ξ, Γ × (Γ → Γ) × 𝖬𝖺𝗒𝖻𝖾(ℑ))
                 𝔈Γfail∗ ≔ 𝜇ℑ. (𝑒 : Γ → 𝖤𝗂𝗍𝗁𝖾𝗋(Ξ, Γ × (Γ → Γ) × 𝖬𝖺𝗒𝖻𝖾(ℑ)))                      (49)
                            × ((𝛾 : Γ) → (𝐥𝐞𝐭 𝖱𝗂𝗀𝗁𝗍(𝛿, 𝑔, 𝑜) = 𝑒(𝛾) 𝐢𝐧 𝑔(𝛿) ≃ 𝛾))
    The witness constrains the 𝖱𝗂𝗀𝗁𝗍 case alone, being vacuous where the pattern does not
match, a raise having nothing to undo, and the 𝑖 that 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀 carries is read at 𝔈Γfail∗ from here
...
Definition 53. Index the steps by 𝑡, so that 𝛾 𝑡 is the state the first 𝑡 of them reach, and write
                                               step𝑡 ≔ 𝑟(𝑛)                                              (50)
for the step taken at 𝛾 𝑡 : the rule 𝑟 it applies, one of the ten, and the name 𝑛 ∈ 𝔑 it applies that
rule at. The sequence starts at a 𝛾 0 with dom(𝐹 0 ) = ⌀, so every fiber comes into existence by
an O-Insert, whether the orchestrator’s or one an iteration takes (Definition 47). A field of 𝛾 𝑡
carries the index as a superscript, so that 𝜃𝑛𝑡 , 𝜔𝑛𝑡 , 𝜎𝑛𝑡 , 𝑔𝑛𝑡 , and 𝑖𝑡𝑛 are the lifecycle state, committed
view, table, accumulator, and remaining iterator of 𝑛 at 𝛾 𝑡 , and 𝐹 𝑡 and 𝜎𝑡 the registry and
coeffect context of 𝛾 𝑡 itself, the 𝐹𝛾 and 𝜎𝛾 of Definition 45 read there. Predicates take the state
as their argument and everything else as a subscript, so installed𝑡𝑛 , target𝑡𝑛 , relied𝑡𝑛 , and quiet𝑡
are the predicates of Definition 46, Definition 49, and Definition 50 at 𝛾 𝑡 . An episode of 𝑛 is a
maximal interval [𝑏, 𝑢] of indices throughout which installed𝑡𝑛 holds. It opens at 𝑏, where 𝑏 >
0 and ¬ installed𝑏−1                   0
                  𝑛 , the empty 𝐹 leaving no fiber installed at the outset; it closes at 𝑢 when
installed𝑢𝑛 and not installed𝑢+1
...
a step taken at 𝛾 𝑡 by a rule acting on 𝑛 is
                          pr1 ∘ 𝑖   at L-Iter, L-Finish, and a landing L-Divert
                  Ψ𝑡 ≔    𝑔         at L-Unload                                                (51)
                         {idΓ       at every other rule
where 𝑖 and 𝑔 are the iterator and the accumulator that 𝜃𝑛𝑡 carries, and the edit edit𝑡 : Γ → Γ is
the bracket read as a function, assigning to the fields it names the values the premises computed
at 𝛾 𝑡 . Both are therefore fixed by step𝑡 together with 𝛾 𝑡 and defined at every state, which is
what lets Theorem 61 and Lemma 71 evaluate them away from 𝛾 𝑡 . Each step factors as
                                       𝛾 𝑡+1 = edit𝑡 (Ψ𝑡 (𝛾 𝑡 ))                               (52)
...
a lookup in the table, and five lookups recur often enough to name.
                   𝑡                    𝑡+1
     rule         𝜃𝑛                   𝜃𝑛                         Ψ𝑡               control fields edited
...
present at 𝛾 𝑡 :
        𝑡+1      𝑡
   1. 𝜎𝑚     ≠ 𝜎𝑚    only where step 𝑡 acts on 𝑚, the write lying inside Ψ𝑡 ;
   2. 𝜔𝑛 comes into existence only where step𝑡 = L-Begin(𝑛) and ceases only where step𝑡 =
      L-Unload(𝑛), so 𝜔𝑛𝑡 is constant for 𝑡 in an episode of 𝑛;
   3. Ψ𝑡 = 𝑔𝑛𝑡 only where step𝑡 = L-Unload(𝑛), and no other step applies 𝑔𝑛 to the state;
   4. ¬ installed𝑡𝑛 ∧ installed𝑡+1
                               𝑛   ⇒ step𝑡 = L-Begin(𝑛),       and     installed𝑡𝑛 ∧ ¬ installed𝑡+1
                                                                                                𝑛   ⇒
           𝑡
      step = L-Unload(𝑛);
...
iterations, or the accumulator 𝑔𝑛𝑡 , which is a composite of the inverses those iterations yielded.
Each of the three is confined to 𝑛 by Definition 48, so Ψ𝑡 writes no field of a fiber present at 𝛾 𝑡
...
𝑛 writes no 𝜃𝑛 , and the entry a registration adds is at a name not present at 𝛾 𝑡 .
...
the 𝜃𝑚 , and dom(𝜎𝛾 ), which Definition 45 computes from the 𝜃𝑚 and the dom(𝜎𝑚 ), and Defin­
ition 33 relates two coeffect contexts only where their domains agree. The remaining premises
read dom(𝐹𝛾 ). None reads a value 𝜎𝛾 (𝑘) otherwise than up to ≃, so no premise separates two
                                                                𝑘
≃-related states.
For the conclusion, 𝛾 𝑡+1 = edit𝑡 (Ψ𝑡 (𝛾 𝑡 )) by Definition 53. The values an edit𝑡 assigns are the
...
𝛾 is a state, well formed where 𝛾 is, and step𝑡 = 𝑟(𝑛) carries 𝛾 𝑡 to 𝛾 𝑡+1 if and only if 𝑟(𝜒(𝑛))
carries 𝜒 ⋅ 𝛾 𝑡 to 𝜒 ⋅ 𝛾 𝑡+1 .
...
names, as target𝑛 and relied𝑛 read the 𝜋𝑚 and the 𝜔𝑚 . A bijection preserves each such compar­
...
Each clause is established at 𝛾 𝑡+1 from all four at 𝛾 𝑡 .
...
(3) By Lemma 54(2) the only rule that writes an 𝜔𝑛 is L-Begin, whose premise 𝜔 = target𝑡𝑛 ≠ ⊥
makes it total on 𝑑𝑛 and valued in dom(𝐹 𝑡 ), target naming providers. By Table 1 the only rule
that shrinks dom(𝐹𝛾 ) is O-Remove, whose premise 𝜃𝑛𝑡 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−) gives ¬ installed𝑡𝑛 , whence
by clause (4) at 𝛾 𝑡 no 𝑚 has 𝜔𝑚
                               𝑡
                                 (𝑘) = 𝑛 for a 𝑘 ∈ 𝑑𝑚 while installed𝑡𝑚 ; and 𝑛 itself carries no 𝜔.
(4) By Lemma 54(2) and (4) the clause can fail at 𝛾 𝑡+1 only where some installed has fallen,
some 𝜔 has been written, or a fiber some 𝜔 names has left dom(𝐹𝛾 ). The last is an O-Remove,
whose removed fiber is not installed and hence, by clause (4) at 𝛾 𝑡 , is named by no 𝜔𝑚
                                                                                       𝑡
                                                                                         of an
                                                                  𝑡
installed 𝑚. The first is an L-Unload of 𝑛, whose premise ¬ relied𝑛 reads
                            ∀𝑚 ≠ 𝑛, 𝑘 ∈ 𝑑𝑚 . installed𝑡𝑚 ⇒ 𝜔𝑚
                                                            𝑡
                                                              (𝑘) ≠ 𝑛
and which writes no 𝜔𝑚 for 𝑚 ≠ 𝑛 and leaves ¬ installed𝑡+1      𝑛 , so the clause holds of 𝑛 as well.
The second is an L-Begin of 𝑛, writing target𝑡𝑛 , whose values are the providers of the keys of
𝑑𝑛 and hence 𝖠𝖼𝗍𝗂𝗏𝖾 at 𝛾 𝑡 ; the step alters no other fiber’s 𝜃, so they are installed at 𝛾 𝑡+1 too. □
...
Definition 60. For 𝑖 ∈ 𝔈Γiter∗ let reach(𝑖) be the least set of iterators containing 𝑖 and closed under
...
acting fiber is not 𝑛. Then
                                   𝑔𝑛𝑢 (𝛾 𝑢 ) ≈ (Ψ𝑡𝑙 ∘ ⋯ ∘ Ψ𝑡1 )(𝛾 𝑏 )                           (56)
That is, applying 𝑛’s accumulator at 𝛾 𝑢 yields, up to the control fields, the state those same
steps would have produced from 𝛾 𝑏 . Reading the right side as the state reached had 𝑛 never
...
an L-Begin, the episode opening by Definition 53, so 𝑔𝑛𝑏 = idΓ by Table 1, the index set is empty,
and the claim is 𝛾 𝑏 ≈ 𝛾 𝑏 . Two facts are used at each step. Since edit𝑡 writes control fields only,
                                           𝛾 𝑡+1 ≈ Ψ𝑡 (𝛾 𝑡 )
and since every map in 𝔐(𝑒𝑛 ) writes no control field but those a registration adds, by Defin­
...
and an L-Unload of 𝑛, and O-Insert and O-Remove read a 𝜃𝑛 that installed𝑢𝑛 denies, leaving
two cases. Where the rule is L-Iter, L-Finish, or a landing L-Divert, Table 1 gives Ψ𝑢 = pr1 ∘ 𝑖𝑢𝑛
and 𝑔𝑛𝑢+1 = 𝑔𝑛𝑢 ∘ ℎ for the inverse ℎ that iteration yields. The witness condition of Definition 51
reads ℎ(Ψ𝑢 (𝛾 𝑢 )) = 𝛾 𝑢 , up to ≈ where the iteration registers a fiber (Lemma 57), and 𝑔𝑛𝑢 carries
≈ by the equation above, so
                            𝑔𝑛𝑢+1 (𝛾 𝑢+1 ) ≈ (𝑔𝑛𝑢 ∘ ℎ)(Ψ𝑢 (𝛾 𝑢 )) = 𝑔𝑛𝑢 (𝛾 𝑢 )
Where the rule is L-Leave, L-Raise, an aborting L-Divert, or an O-Retire of 𝑛, Table 1 gives
Ψ𝑢 = idΓ and 𝑔𝑛𝑢+1 = 𝑔𝑛𝑢 , so the same equation holds with ℎ = idΓ . Either way the induction
hypothesis carries over with the index set unchanged, which is the computation of Theorem 7
one step at a time.
Let step 𝑢 act on 𝑚 ≠ 𝑛. Then 𝑔𝑛𝑢+1 = 𝑔𝑛𝑢 by Table 1, and Ψ𝑢 ∈ 𝔐(𝑒𝑚 ), or Ψ𝑢 = idΓ where the
rule is an orchestration rule, so independence gives
                             𝑔𝑛𝑢 (𝛾 𝑢+1 ) ≈ 𝑔𝑛𝑢 (Ψ𝑢 (𝛾 𝑢 )) = Ψ𝑢 (𝑔𝑛𝑢 (𝛾 𝑢 ))
...
as in Theorem 61,
                                    𝛾 𝑢+1 ≈ (Ψ𝑡𝑙 ∘ ⋯ ∘ Ψ𝑡1 )(𝛾 𝑏 )                             (57)
A fiber removed by O-Remove leaves nothing behind either, its premise admitting only 𝜃𝑛 =
𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−).
Proof. By Lemma 54(4) step 𝑢 is an L-Unload of 𝑛, whose Ψ𝑢 is 𝑔𝑛𝑢 by Lemma 54(3), so 𝛾 𝑢+1 ≈
𝑔𝑛𝑢 (𝛾 𝑢 ) and Theorem 61 applies. Neither the statement nor ≈ mentions 𝜁, which by Table 1 is
...
Theorem 63. (Ordering.) A fiber begins a transition only where its dependencies are provided:
                                  step𝑡 = L-Begin(𝑚) ⇒ 𝛾 𝑡 ⊧ 𝑑𝑚                                  (58)
                                                 ′
Let further [𝑏′ , 𝑢′ ] be an episode of 𝑚 with 𝜔𝑚
                                                𝑏
                                                  (𝑘) = 𝑛 for some 𝑚 ≠ 𝑛 and 𝑘 ∈ 𝑑𝑚 , let [𝑏, 𝑢] be
the episode of 𝑛 containing 𝑏 , and let 𝑡 range over [𝑏′ , 𝑢′ ]. Then
   ...
   2. 𝑏 < 𝑏′ , and 𝑢′ < 𝑢 if [𝑏, 𝑢] closes;
                                       ′
   3. 𝑘 ∈ dom(𝜎𝑛𝑡 ) and 𝜎𝑛𝑡 (𝑘) = 𝜎𝑛𝑏 (𝑘).
Proof. The first claim is the premise target𝑡𝑚 ≠ ⊥ of L-Begin, which by Definition 46 gives 𝛾 𝑡 ⊧
𝑑𝑚 .
(1) is Lemma 54(2).
                                           ′           ′                                          ′
For (2), the L-Begin at 𝑏′ − 1 writes 𝜔𝑚 𝑏
                                           = target𝑏𝑚−1 , whose values are providers, so 𝜃𝑛𝑏 =
𝖠𝖼𝗍𝗂𝗏𝖾(−, −); the L-Begin at 𝑏 − 1 leaves 𝜃𝑛𝑏 = 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−, −, −), so 𝑏 ≠ 𝑏′ and hence 𝑏 < 𝑏′ ,
both episodes opening by Definition 53. Let [𝑏, 𝑢] close and suppose 𝑢 ≤ 𝑢′ . Then 𝑢 ∈ [𝑏′ , 𝑢′ ],
so installed𝑢𝑚 and, by (1), 𝜔𝑚
                             𝑢
                               (𝑘) = 𝑛; that is relied𝑢𝑛 , which the L-Unload at 𝑢 denies. Hence
 ′
𝑢 < 𝑢.
                                     ′                 ′
For (3), 𝑛 is the provider of 𝑘 at 𝛾 𝑏 , so 𝑘 ∈ dom(𝜎𝑛𝑏 ). No L-Unload of 𝑛 falls in [𝑏′ , 𝑢′ ]: where
[𝑏, 𝑢] closes it falls at 𝑢 > 𝑢′ by (2), and where it does not, Lemma 54(4) leaves 𝑛 with no L-
                          ′
Unload at all. Since 𝜃𝑛𝑏 = 𝖠𝖼𝗍𝗂𝗏𝖾(−, −), Table 1 therefore leaves L-Leave as the only rule 𝑛 can
...
Theorem 64. (Resolution coherence.) Let an episode [𝑏, 𝑢] of 𝑛 open at 𝑏 with 𝜔𝑛𝑏 = 𝜔. Then
...
transition runs against the one resolution 𝜔:
                   ∀𝑡 ∈ [𝑏, 𝑟]. step𝑡 ∈ {L-Iter(𝑛), L-Finish(𝑛)} ⇒ target𝑡𝑛 = 𝜔                  (59)
Where the fiber leaves that interval, so that 𝑟 < 𝑢, exactly one of the following holds:
  1. step𝑟 = L-Finish(𝑛) and 𝜃𝑛𝑟+1 = 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔);
  2. step𝑟 ∈ {L-Divert(𝑛), L-Raise(𝑛)}, and the episode closes at some 𝑢 > 𝑟 with 𝛾 𝑢+1 ≈
     (Ψ𝑡𝑙 ∘ ⋯ ∘ Ψ𝑡1 )(𝛾 𝑏 ) as in Corollary 62.
...
acting on 𝑛 and
                               𝑉 (𝑛) ≔ |{𝑡 : target𝑡𝑛 ≠ target𝑡+1
                                                              𝑛 }|                             (61)
for the number of times its target view turns. Then
    1. (No deadlock.) ¬ quiet𝑡 implies that some lifecycle rule applies at 𝛾 𝑡 ;
...
tion 49. Reading Table 1 against the four kinds it can then be:
    • 𝜃𝑛𝑡 = 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) with target𝑡𝑛 ≠ ⊥: L-Begin applies;
     • 𝜃𝑛𝑡 = 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−, −, 𝜔𝑛 ) with target𝑡𝑛 = 𝜔𝑛 : whichever of L-Iter, L-Finish, and L-Raise
        the value of 𝑖𝑡𝑛 (𝛾 𝑡 ) selects applies;
     • 𝜃𝑛𝑡 = 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−, −, 𝜔𝑛 ) with target𝑡𝑛 ≠ 𝜔𝑛 : L-Raise applies if 𝑖𝑡𝑛 (𝛾 𝑡 ) raises, and other­
        wise L-Divert does, landing that iteration rather than aborting it;
     • 𝜃𝑛𝑡 = 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔𝑛 ) with target𝑡𝑛 ≠ 𝜔𝑛 : L-Leave applies.
                                                               𝑡
Let no fiber be of any of these kinds, leaving some 𝑚0 with 𝜃𝑚   0
                                                                   = 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−, −, −). Construct
                                                               𝑡
𝑚0 , 𝑚1 , … as follows: given 𝑚𝑗 in 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀, either ¬ relied𝑚𝑗 , in which case L-Unload applies
to 𝑚𝑗 and the construction stops, or there are 𝑚𝑗+1 ≠ 𝑚𝑗 and 𝑘𝑗 with installed𝑡𝑚𝑗+1 and
  𝑡
𝜔𝑚  𝑗+1
...
𝑚𝑗+1 . Moreover target𝑡𝑚𝑗+1 ≠ 𝜔𝑚
                               𝑡
                                 𝑗+1
                                     : an 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 fiber is outside the union defining 𝜎𝛾 , so 𝑘𝑗
    𝑡
at 𝛾 is unprovided or provided by a fiber other than 𝑚𝑗 . Were 𝑚𝑗+1 in 𝖠𝖼𝗍𝗂𝗏𝖾 or 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀
...
(A) Over a maximal interval on which target𝑡𝑛 is constant at 𝜔∗ , at most 𝐾 + 4 steps act on 𝑛.
...
L-Divert or L-Leave falls in the interval, the 𝜔 that the L-Begin writes being target𝑡𝑛 = 𝜔∗ itself,
and at 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔∗ ), at 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) with 𝜔∗ = ⊥, and at 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜉) no rule applies at all.
(B) If target𝑡𝑛 ≠ target𝑡+1
...
and 𝑝 alone.
Proof. Order the names of dom(𝐹𝛾 ) by the index of the step that registered each, which Defin­
...
let steps 𝑡 and 𝑡 + 1 act on distinct fibers 𝑚 and 𝑛.
    1. If both apply an activation rule, namely L-Begin, L-Iter, or L-Finish, and step 𝑡 + 1 is
       applicable at 𝛾 𝑡 , then step 𝑡 is applicable at the state step 𝑡 + 1 produces from 𝛾 𝑡 , and the
       two orders reach the same 𝛾 𝑡+2 .
...
rule writing a 𝜏 . Its resolution half cannot move either: step 𝑡 + 1 being applicable at 𝛾 𝑡 puts
...
relaxes, whence its applicability at 𝛾 𝑡+1 gives its applicability at 𝛾 𝑡 ; here step 𝑡 not registering
𝑛 is what keeps 𝑛 present at 𝛾 𝑡 where O-Retire and O-Remove require it.                              □
Lemma 72. (Deletion.) Let the sequence of steps be pairwise independent, let every compo­
nent be total on its provision (Definition 69), let it reach a quiescent 𝛾 𝑇 at which no fiber is
...
on a name of 𝑅, leaves a sequence of steps reaching a state ≈-equal to 𝛾 𝑇 and ≃-equal to it
outside 𝑅.
...
act on fibers other than 𝑛. Corollary 62 reads
                                     𝛾 𝑢+1 ≈ (Ψ𝑡𝑙 ∘ ⋯ ∘ Ψ𝑡1 )(𝛾 𝑏 )
whose right side is what the surviving steps of [𝑏, 𝑢] produce on their own, 𝛾 𝑏−1 ≈ 𝛾 𝑏 and their
...
fiber being failed, and which it held at 𝛾 𝑏−1 .
An invariant carries the suffix. Write 𝛾 ′𝑡 for the state the surviving steps reach at the point
corresponding to 𝑡. We claim, for every 𝑡 > 𝑢, that 𝛾 𝑡 ≈ 𝛾 ′𝑡 , that every name of 𝑅 is vestigial at
𝛾 𝑡 and absent from 𝛾 ′𝑡 , and that the two states agree on every field of every name outside 𝑅. At
...
so it is open at 𝛾 𝑇 , where quiet gives 𝜔𝑚 = target𝑇𝑚 and Lemma 70 puts its values among the
...
Theorem 73. (Confluence.) Let a sequence of steps reach a quiescent 𝛾 𝑇 at which no fiber is
...
(Definition 69), and let 𝐴 be as in Definition 67. Then
    1. (Canonical form.) 𝛾 𝑇 is reached, up to the names whose entries the reduction withdraws,
       from 𝛾 0 by a sequence that takes the same orchestration steps in their original order,
    ...
    2. (Confluence.) Any two such sequences from 𝛾 0 taking the same orchestration steps reach
...
open at 𝛾 𝑇 , which by quiet𝑇 and Lemma 70 are one episode of each fiber of 𝐴.
...
𝛾 𝑇 ; and ⊲ relates it to 𝑛 through its parent pointer, so by maximality it has no closing one either.
The lemma removes the episode, together with the steps of the names it registered, leaving 𝛾 𝑇
where it was up to those names. The measure drops by one, so no closing episode remains.
A fiber outside 𝐴 takes no lifecycle step. It has no open episode at 𝛾 𝑇 , by Lemma 70 and quiet𝑇 ,
...
⌀ and 𝜋𝑛1 = 𝗋𝗈𝗈𝗍, since Definition 67 puts a provider of a key of 𝑑𝑛1 and the fiber registering 𝑛1
in 𝐴 while ⊲ puts both below 𝑛1 . So target𝑛1 reads no field of another fiber and, no orchestration
step remaining to write 𝜏𝑛1 and no fiber below 𝑛1 remaining to retire it, is constant. Every
step acting on 𝑛1 is an activation step, no episode closing, and its remaining premises read
𝜃𝑛1 and 𝑖𝑛1 , which by Table 1 only 𝑛1 writes; each is therefore applicable at every earlier state,
...
access to an opaque slot on the context, rather than indexing into a string-keyed map.
 ...
 𝛾∈Γ                                   the context tree together with everything the running system
                                       has touched
 𝔈Γ , 𝔈Γiter                           Effect callback returning / yielding inverses
...
ationally, ctx.effect is the realization of effectiter
                                                  Γ (Definition 52): it takes a callback of type 𝔈Γ
                                                                                                   iter
                 iter
and lifts it to 𝔈𝜕Γ   , yielding a dispose closure that, when invoked, recovers the effect. Cordis
accepts both 𝔈Γ and 𝔈Γiter through this one operation (ad-hoc polymorphism); we take the
iterator form as representative, since a plain effect function is the degenerate iterator that yields
a single inverse. What the operation does not check is the witness that 𝔈Γ∗ carries: the callback
...
𝑓 after 𝑔, and id for the no-op; prepending each new inverse therefore yields LIFO recovery.
 ...
 18        return dispose
    The engine execute drives the callback as an effect iterator (𝔈Γiter , Definition 51) and folds
...
is the recursive structure of 𝜕 2 Γ. The component level (Section 5.1.3) reuses the same execute
